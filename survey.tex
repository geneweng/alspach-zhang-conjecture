...
Here is a concrete use of the group action that remains to be justified in general.  Let $M(P)$ be the forced matching upstairs and let $g\in G$.  The symmetric difference $M(P)\mathbin\triangle gM(P)$ is a union of even alternating circuits.  Flipping any one circuit $D$ gives another perfect matching $M'=M(P)\mathbin\triangle D$.  Its $a$-edge support $P'$ is locally valid provided it does not select all five positions at any quotient vertex; matching the remaining vertices of a pentagon forces precisely the one-position or consecutive-three-position patterns.  Therefore it would suffice to prove that, whenever $H_P$ has an odd-sign circuit, some translate and some alternating circuit give a locally valid $P'$ with strictly fewer odd-sign circuits.  Starting with Proposition~\ref{prop:starting-support}, strict descent would terminate at a colouring.  This repair statement is \emph{not proved}; the experiment below only checks several small instances.
...
\item (Colour support and signed circuits.)  For $s=5$, can the Cayley action be used to choose a support $P$ satisfying Proposition~\ref{prop:colour-support} with every circuit of $H_P$ sign-even?  Proposition~\ref{prop:starting-support} guarantees a locally valid starting support.  Can one always reduce the number of odd-sign circuits by flipping a component of $M(P)\mathbin\triangle gM(P)$ for a suitable $g\in G$, while preserving local validity?  This would prove the order-five case by descent.  Proposition~\ref{prop:a5-descent-check} verifies this rule completely on $A_5$, even using paired circuits for involutions, and larger finite tests succeed, but no general decreasing move has been proved.  Can the regular-action divisibility of Proposition~\ref{prop:orbit-divisibility} and the local agreement gained in Lemma~\ref{lem:involution-exchange} force one?  Example~\ref{ex:symmetry-barrier} shows that monotone growth of the matching stabiliser, or complete symmetrisation inside the union with a new involution translate, cannot be demanded at every step.
\end{enumerate}
