...
\end{proposition}
\begin{proof}
The affine group on $A$ has order $4\cdot6=24$ and induces all $24$ permutations of its four points, proving the first statement.  The equality of the upper endpoints of the two displayed pairs gives \eqref{eq:matching-collision}.  If $L$ has order two, $N=I+L$ has rank one.  When $y_e+c\notin\operatorname{im}N$, collision is impossible.  Otherwise $Nz_e=y_e+c$ describes one affine line in $A$, and avoiding it means that $z_e$ belongs to the other affine line, a single affine linear equation.
...
For that separated transvection witness the compatibility system has nullity $8$.  Proposition~\ref{prop:cdc-rigidity} therefore identifies five genuine degrees of freedom in $\mathcal K_f$ beyond global translation.  This is enough for the particular matching found by SAT, but the outstanding proof problem is to control the restriction of this code to the collision equations without first knowing a Tait colouring.
...
\item (Weighted cut code.)  Can the generator-separated flow be chosen so that $\mathcal K_f$ from \eqref{eq:weighted-cut-code} has enough controlled restriction to satisfy one transvection matching's collision equations?  Proposition~\ref{prop:cdc-rigidity} shows that this is exactly the effective freedom available after the flow and local triangles have been fixed.
\end{enumerate}
