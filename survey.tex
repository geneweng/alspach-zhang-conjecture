...
It does give a concrete way to manufacture layer potentials.  Recall that the cyclic order of the $a$-edges incident with a vertex of $Q$ is inherited from the corresponding $x$-cycle.
...
\item (Weighted cut code.)  Can the quotient flow be chosen with enough vertex-disjoint consecutive monochromatic cycles to make Proposition~\ref{prop:mono-cycle} useful, and can the resulting lifts be positioned so that the syndrome criterion \eqref{eq:syndrome-criterion} holds for one transvection matching?  More generally, can the extendable layer-potential space $\mathcal R_f$ in Proposition~\ref{prop:layer-potential} be controlled directly?  Propositions~\ref{prop:cdc-rigidity} and~\ref{prop:transvection-syndrome} show that the map from $\mathcal K_f$ to the dangerous-edge equations is exactly the effective freedom available after the flow and local triangles have been fixed.
\end{enumerate}
