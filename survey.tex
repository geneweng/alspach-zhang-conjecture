...
\[
 Z_k(Q)=[t^{|G|/10}]E_k(t).
\]
Thus the target is not merely the usual edge-colouring count: it is its lowest possible $a$-edge coefficient.  The $\Omega$ polynomial of Ellis-Monaghan, Kauffman and Moffatt \cite{EllisMonaghanKauffmanMoffatt18} does count edge colourings of arbitrary ribbon graphs, while the embedded Penrose polynomial at $3$ has that interpretation only in the plane.  Formula~\eqref{eq:signed-loop-sum} identifies the additional coefficient and sign filter that a polynomial proof would have to control.  The orbit formula is the promising new point at which dart-regularity can enter; the local transition identity alone cannot prove non-vanishing, since the Petersen control has five quotient matchings and $Z_k=0$.
...
The same exhaustive data verify Proposition~\ref{prop:signed-loop-sum} in three independent forms: solving each affine circuit system, multiplying its signed-loop factors, and grouping its nonnegative contributions into $G$-orbits.  In the order just used, the exact values of $Z_k(Q)$ are
\[
