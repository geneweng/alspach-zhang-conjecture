...
selected non-solvable groups of order $\le 40\,000$ & Section~\ref{sec:computation}\\
\bottomrule
\end{tabular}
...
Conjecture~\ref{conj:AZ} holds for every cubic Cayley graph on each of the groups listed in Table~\ref{tab:computation}; in particular for every non-abelian simple group of order at most $10\,000$, for $\PSL(2,q)$ with $q\le 43$, for $\PGL(2,q)$ with $q\le31$, for $A_n$ with $n\le8$ and $S_n$ with $n\le7$, and for the groups $M_{10}$, $M_{11}$, $\PSL(3,3)$, $\PSL(3,3).2$, $\operatorname{PSU}(3,3)$, $G_2(2)=\operatorname{PSU}(3,3).2$, $\PSigmaL(2,16)$, the three index-two extensions of $\PSL(2,25)$ and $A_5\wr\ZZ_2$.
\end{theorem}
